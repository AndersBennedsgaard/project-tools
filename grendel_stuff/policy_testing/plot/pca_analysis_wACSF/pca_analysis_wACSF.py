...
legend_labels = [direc.rstrip('/').lstrip(common_prefix) for direc in directories]
...
recalc = False
try:
    parameters = np.load(main_dir + 'files/kmeans_pca_parameters.npy').tolist()
...
remove_ticks(ax)
# for (i, label), centroid in zip(enumerate(labels), centroids):
#     dat = results[n_dat[i]: n_dat[i + 1], :]
#     plt.plot(dat[:, 0], dat[:, 1], '.', markersize=2, label=label)
